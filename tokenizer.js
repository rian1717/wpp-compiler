...
/**
 * Calculates the 1-based column number for a character index in the source code.
 * @param {string} sourceCode - The full source code of the W++ program.
 * @param {number} index - The character index within the source code.
 * @returns {number} The 1-based column number of the character.
 */
...
/**
 * Tokenizes the raw source code of a W++ program into a stream of tokens.
 * @param {string} sourceCode - The source code to analyze.
 * @returns {Array<Object>} An array of token objects containing category, type, value, line, and col.
 */
...
/**
 * Processes a scanned token array to calculate compilation metadata and statistics.
 * @param {Array<Object>} tokens - The full token stream from the tokenizer.
 * @param {string} sourceCode - The original source code of the file.
 * @param {string} filename - The name of the file being compiled.
 * @returns {Object} An object containing token statistical details.
 */
...
function padR(s, n) {
  ...
  const targetLen = Math.max(0, parseInt(n) || 0);
  return str.slice(0, targetLen).padEnd(targetLen);
}

function padL(s, n) {
  const str = s === undefined || s === null ? '' : String(s);
  const targetLen = Math.max(0, parseInt(n) || 0);
  return str.slice(0, targetLen).padStart(targetLen);
}

function centerText(text, width) {
  const str = text === undefined || text === null ? '' : String(text);
  const targetWidth = Math.max(0, parseInt(width) || 0);
  const pad = Math.max(0, Math.floor((targetWidth - str.length) / 2));
  return ' '.repeat(pad) + str;
}

/**
 * Compiles analyzed token statistics into a human-readable text report.
 * @param {Object} stats - The statistics object generated by generateAllStats.
 * @returns {string} The plain text statistical summary report.
 */
